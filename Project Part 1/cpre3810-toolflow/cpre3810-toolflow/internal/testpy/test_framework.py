...
class SimWorker:
    ...
    def simulate(self,asm_path):
        ...
        compare, compare_out, inst, cycles = compare_dumps(self.options, self.container, self.container / "ms.trace", self.container / 'rars.trace')
...
def check_vhdl_present():
    ...
    expected = {'tb.vhd','mem.vhd','RISCV_Processor.vhd'}
...
def check_project_files_exist():
    ...
    '''
    expected_files = [
        'internal/Rars/RARS_CPRE381.jar',
...
def warn_tb_checksum():
    ...
    '''
    expected = {
        b'U\xdf~3\xf9\xe1\xedb\x1a\xb7MEX\xe0MO'
        }
...
def compare_dumps(options, dir, student_dump, rars_dump):
    '''
    ...
    dc = DumpCompare(student_dump, rars_dump, mismatches, outfunc=compare_out_function)
